...
TEST(QuickSortTest, IntegerEvenSize)
{
	...
	QuickSort(SortArray, 0, 49);
	EXPECT_THAT(ExpectedArray, testing::ContainerEq(SortArray));
}

TEST(QuickSortTest, IntegerOddSize)
{
	...
	QuickSort(SortArray, 0, 48);
	EXPECT_THAT(ExpectedArray, testing::ContainerEq(SortArray));
}

TEST(QuickSortTest, CharEvenSize)
{
	char SortString[] = "TwinHits";
	char ExpectedString[] = "HTiinstw";
	QuickSort(SortString, 0, 7);
	EXPECT_TRUE(CStringCompare(SortString, ExpectedString));
}

TEST(QuickSortTest, CharOddSize)
{
	char SortString[] = "TwinHit";
	char ExpectedString[] = "HTiintw";
	QuickSort(SortString, 0, 6);
	EXPECT_TRUE(CStringCompare(SortString, ExpectedString));
}
